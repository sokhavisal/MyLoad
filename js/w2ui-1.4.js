...
/////-------------------------
                                //if(['int','float'].indexOf(search.type) != -1){
...
//                                if (tmp.indexOf(val1) == -1) fl++;
            ...
            if (arguments.length == 0) {
                ...
                for (var s in this.searches) {
                    ...
                    if ((value1 != '' && value1 != null) || (typeof value2 != 'undefined' && value2 != '')) {
                        ...
                        } else if (operator == 'More than' && typeof value1 == 'string') {
                            $.extend(tmp, { value: value1.split(',') });
                        } else if (operator == 'Less than' && typeof value1 == 'string') {
                            $.extend(tmp, { value: value1.split(',') });
                        } else {
            ...
            for (var i = 0; i < this.searches.length; i++) {
                ...
                if (['int', 'float', 'money', 'currency', 'percent', 'date', 'time'].indexOf(s.type) != -1) {
                    var operator =  '<select id="grid_'+ this.name +'_operator_'+ i +'" '+
                        ...
                        '    <option value="is">'+ w2utils.lang('is') +'</option>'+
                        (['int'].indexOf(s.type) != -1 ? '<option value="More than">'+ w2utils.lang('More than') +'</option>' : '') +
                        (['int'].indexOf(s.type) != -1 ? '<option value="Less than">'+ w2utils.lang('Less than') +'</option>' : '') +
                ...
                if (['enum'].indexOf(s.type) != -1) {
                    var operator =  '<select id="grid_'+ this.name +'_operator_'+ i +'" onclick="event.stopPropagation();">'+
                        '    <option value="More than">'+ w2utils.lang('More than') +'</option>'+
                        '    <option value="Less than">'+ w2utils.lang('Less than') +'</option>'+
                        '</select>';
                }
                html += '<tr>'+
        ...
        initOperator: function (el, search_ind) {
            ...
            if ($(el).val() == 'More than' || $(el).val() == 'Less than') { fld1.w2field('clear'); } else { fld1.w2field(search.type); }
        ...
        initSearches: function () {
            ...
            for (var s in this.searches) {
                ...
                switch (search.type) {
                    ...
                    case 'date':
                    case 'time':
                        if (sdata && sdata.type == 'int' && ['More than', 'Less than'].indexOf(sdata.operator) != -1) break;
                        $('#grid_'+ this.name +'_field_'+s).w2field(search.type, search.options);
                ...
                if (sdata != null) {
                    if (sdata.type == 'int' && ['More than', 'Less than'].indexOf(sdata.operator) != -1) {
                    ...
                    if (!$.isArray(sdata.value)) {
                        ...
                        if (['More than', 'Less than'].indexOf(sdata.operator) != -1) {
